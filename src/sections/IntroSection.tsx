...
const IntroSection: Component = () => {
  ...
  return (
    <div ref={wrapper} class={styles.Intro}>
      <For each={steps}>{AnimatedIntroSection}</For>
      <CrossHair />
      <IntroGridOverlay />
    </div>
  );
...
export const getTitleClass = (title: string) =>
...
const AnimatedIntroSection = (step: Step) => (
  <section>
    <div class="outer">
      <div class="inner">
        <div
          ...
          {!step.title && (
            <>
              {steps.map(({ title }, index) => (
                <h2
                  class={[getTitleClass(title), index < 3 ? "" : "dimmed"].join(
                    " "
                  )}
                >
                  {title}
